...
电机停止后可从右上角进入。工程师模式的 UART5/RS485 仅用于 Debug 固件的状态、日志、
ADC 波形和纯软件 CAN 编解码自检；CAN 参数、长稳和协议诊断继续使用 USB-CAN。高级 Iq、
...
5. 需要协议、参数和长稳诊断时进入高级模式；需要 UART5 遥测时必须使用 Engineering Debug 固件。
