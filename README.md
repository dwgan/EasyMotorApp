...
- 速度斜坡保持 10 motor rpm/s，因此 100 rpm 档约需 10 秒达到设定值，应配合“一直转”演示；
- CAN 自动执行 `Type 18 选择 run_mode=2 → Type 3 → 等待 Type 2 MOTOR →`
  `周期写 0x700A → Type 4 → 恢复 run_mode=0`，用户不需要理解对齐和 MCI；
...
3. 选择 5/30/100 motor rpm，点击正转或反转；默认 5 秒后自动发送 Type 4 停止。
4. 只有明确勾选“一直转”才持续刷新 `0x700A` 速度参考，点击停止、断线、故障或 MCU 看门狗都会结束。
5. 需要波形时保持 CAN 在线并进入高级模式，在“RS485 Debug”页选择另一 COM 口和与固件一致的波特率；连接后打开波形窗口，再切换到“CAN Control”页使用同样受限的 5/30/100 rpm 控制。波形独立窗口会继续显示。UART5 遥测必须使用 Engineering Debug 固件。普通模式观察三相波形，包络模式保留每个统计窗口的三相峰值；“单路全采样”使用 128 点二进制分块，在当前 50 kHz FOC 配置下连续传输选定 U/V/W 一路的全部控制周期采样。横轴时间窗可选择 30 ms、200 ms、1 s、2 s 或 5 s；长时间窗按屏幕像素保留每列 min/max 后绘制，因此扩大周期范围不会隐藏窄尖峰，CSV 仍保存完整接收样本。
...
- 演示速度固定为 5/30/100 motor rpm，定时运行默认 5 秒；演示使用独立
  `run_mode=2` 速度 PI，不复用或重新解释 MIT Type 1。
- 速度环输出限制 ±100 LSB，固件还有 150 LSB 软件限流和独立 TIM1
  硬件过流保护（±15 A 窗口）。
- EasyMotorApp 以 250 ms 刷新 `0x700A`；停止会发送 Type 4 并在 IDLE
  恢复 `run_mode=0`，MCU 自身的 1000 ms 命令看门狗仍独立生效。
